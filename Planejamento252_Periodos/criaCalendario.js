function criaCalendario() {
  ...
  for (let i=0; i<tabela.getRowCount(); i++) {
    ...
    let optEnfase = tabela.get(i, optQualEnfase);
    let optOficinaEnfase = tabela.get(i, optOfiQualEnfase);
    if (periodo.includes(qualPeriodo)||periodo.includes(optPratExp)||
      periodo.includes(optDesenhoEnfase)||periodo.includes('OPTATIVA GERAL DE ENFASE')||
      periodo.includes(optHist)||periodo.includes('OPTATIVA DE OFICINA DE ENFASE')) {
      if (periodo.includes('OPTATIVA GERAL DE ENFASE') && optEnfase !='TRUE') {
        //nothing
      } else if (periodo.includes('OPTATIVA DE OFICINA DE ENFASE') && optOficinaEnfase !='TRUE') {
        //nothing
      }else{
